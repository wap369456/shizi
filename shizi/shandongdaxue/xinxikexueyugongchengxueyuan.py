...
def get_data(address):
    ...
    college_uuid = '1353e1e81aa54c769e7fda8026a3e13c'
    try:
...
if __name__ == '__main__':
    ...
    cursor = conn.cursor()
    url = 'https://www.ise.sdu.edu.cn/szll/jzg.htm'
    driver.get(url)
    soup = BeautifulSoup(driver.page_source, 'lxml')
    a_s = soup.find('div', attrs={'id': 'vsb_content_1727_u51'}).find_all('a')
    for a in a_s:
        try:
            name_a = a.get_text()
            if '../..' in a['href']:
                href = a['href'].replace('../..', 'https://www.ise.sdu.edu.cn')
            else:
                if '..' in a['href']:
                    href = a['href'].replace('..', 'https://www.ise.sdu.edu.cn')
                else:
